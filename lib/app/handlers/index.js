...
exports.getPostSlugs = function (req, res) {
  ...
  models.post.find({}, { slug: 1 }, function(err, slugsObj){
    ...
    if (err) {
      ...
      log.warn({'api_action': 'get slugs', 'api_status': 'success', 
        'warn': 'no slugs found'});
...
exports.getPost = function (req, res) {
  ...
  if(!slug){
    log.error({'api_action': 'get', 'api_status': 'start', 
      'error': 'no slug provided for get'});
    res.status(400).send('no slug provided for get');
    return;
  }
  log.info({'post_slug': slug, 'api_action': 'get', 
            'api_status': 'start'});
  models.post.findOne({'slug': slug}, function(err, post){
    if (err) {
      log.error({'slug': slug, 'api_action': 'get', 
                'api_status': 'failure', 'error': err});
      res.status(400).send('Internal error, see log for details');
    } else if (!post){ 
      log.warn({'post_title': post.title, 'api_action': 'get', 
                'api_status': 'success', 'warn': 'post not found'});
      res.status(404).send('not found');
    } else {
      log.info({'post_title': post.title, 'api_action': 'get', 
                'api_status': 'success'});
...
exports.removePost = function (req, res) {
  ...
  if(!slug){
    log.error({'api_action': 'delete', 'api_status': 'start', 
      'error': 'no slug provided for delete'});
  ...
  models.post.findOneAndRemove({'slug': slug}, function(err) {
    if (err){
      log.error({'api_action': 'delete', 'api_status': 'failure', 
        'post_slug': slug, 'msg': err});
      res.status(500).send('Something went wrong! Check the logs');
    } else {
      log.info({'api_action': 'delete', 'api_status': 'success', 
        'post_slug': slug, 'msg': 'successfully deleted'});
...
exports.createPost = function (req, res) {
  ...
  newPost.save(function(err, post){
    if (err){ 
      log.error({'post': post, 'api_action': 'create', 
                 'api_status': 'failure', 'error': err});
      res.status(400).send(err);
    } else {
      log.info({'post': post, 'api_action': 'create', 
                'api_status': 'sucess'});
